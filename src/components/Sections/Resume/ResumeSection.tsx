...
interface ResumeSectionProps {
  ...
  imageSrc?: string | StaticImageData; // optional image under the title
  imageAlt?: string;
  imageWidth?: number;
  imageHeight?: number;
}
  ...
  ({
    ...
    return (
      <div className={`w-full bg-gray-800 dark:bg-gray-900 ${className}`}>
        ...
        >
          {/* Left column: title + optional image */}
          <div className="col-span-1 flex flex-col items-center md:items-start gap-y-4">
            ...
            {/* Optional image under the title */}
            {imageSrc && (
              <Image
                alt={imageAlt || title}
          ...
          <div className="col-span-1 md:col-span-3 flex flex-col text-white dark:text-white space-y-4">
            {children}
          </div>
        </div>
      </div>
